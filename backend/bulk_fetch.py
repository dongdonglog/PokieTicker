...
Phase 1 of the A-share migration only requires OHLC to be populated.
News fetching is currently stubbed in `backend.ashare.client`.
"""
...
def fetch_ticker_name(symbol: str) -> str:
    ...
    try:
        query = normalize_symbol(symbol).split(".")[0]
        matches = search_tickers(query, limit=5)
